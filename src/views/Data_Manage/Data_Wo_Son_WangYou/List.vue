          ...
          <a-col :md="4" :sm="24">
            <a-form-item label="查询类别">
              <a-select allowClear v-model="queryParam.condition">
                ...
                <a-select-option key="ReportType">通报类型</a-select-option>
...
const columns = [
  ...
  { title: '通报类型', dataIndex: 'ReportType', width: '10%' },
  { title: '区域名称', dataIndex: 'AreaName', width: '10%' },
  { title: '告警名称', dataIndex: 'AlarmName', width: '10%' },
  { title: '告警开始时间', dataIndex: 'AlarmStartTime', width: '10%' },
  { title: '告警恢复时间', dataIndex: 'AlarmEndTime', width: '10%' },
  { title: '工单时限', dataIndex: 'TimeLimit', width: '10%' },
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      this.loading = true
      this.$http
        .post('/Data_Manage/Data_Wo_Son_WangYou/GetDataList', {
      ...
      this.$confirm({
        ...
        onOk() {
          return new Promise((resolve, reject) => {
            thisObj.$http.post('/Data_Manage/Data_Wo_Son_WangYou/DeleteData', ids).then(resJson => {
              resolve()
