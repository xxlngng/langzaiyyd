          ...
          <a-col :md="4" :sm="24">
            <a-form-item label="查询类别">
              <a-select allowClear v-model="queryParam.condition">
                ...
                <a-select-option key="AreaName">区域名称</a-select-option>
                <a-select-option key="SiteName">站址名称</a-select-option>
                <a-select-option key="HandleMajor">受理专业</a-select-option>
                <a-select-option key="LatestMsg">最新回复消息</a-select-option>
                <a-select-option key="OptimizeArea">优化片区</a-select-option>
                <a-select-option key="GridLeader">网格组长</a-select-option>
                <a-select-option key="ImpUrl">图片地址</a-select-option>
                <a-select-option key="FaultRemark">隐患描述</a-select-option>
...
const columns = [
  ...
  { title: '区域名称', dataIndex: 'AreaName', width: '10%' },
  { title: '站址名称', dataIndex: 'SiteName', width: '10%' },
  { title: '受理专业', dataIndex: 'HandleMajor', width: '10%' },
  { title: '恢复时间', dataIndex: 'RecoveryTime', width: '10%' },
  { title: '最新回复消息', dataIndex: 'LatestMsg', width: '10%' },
  { title: '优化片区', dataIndex: 'OptimizeArea', width: '10%' },
  { title: '网格组长', dataIndex: 'GridLeader', width: '10%' },
  { title: '时限（min）', dataIndex: 'TimeLimit', width: '10%' },
  { title: '图片地址', dataIndex: 'ImpUrl', width: '10%' },
  { title: '隐患描述', dataIndex: 'FaultRemark', width: '10%' },
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      this.loading = true
      this.$http
        .post('/Data_Manage/Data_Wo_Son_WuXian/GetDataList', {
      ...
      this.$confirm({
        ...
        onOk() {
          return new Promise((resolve, reject) => {
            thisObj.$http.post('/Data_Manage/Data_Wo_Son_WuXian/DeleteData', ids).then(resJson => {
              resolve()
